...
export default function Launcher({
	...
	function handleMediaKeys(
		...
		if (modifier === Gdk.ModifierType.CONTROL_MASK) {
			if (keyval === Gdk.KEY_l) {
				...
				if (loopStatus === Mpris.Loop.NONE)
					return player.set_loop_status(Mpris.Loop.PLAYLIST);
				if (loopStatus === Mpris.Loop.PLAYLIST)
					return player.set_loop_status(Mpris.Loop.TRACK);
				if (loopStatus === Mpris.Loop.TRACK)
					return player.set_loop_status(Mpris.Loop.NONE);
			}
	...
	const [controlsLeftBoxVisible, setControlsLeftBoxVisible] =
		createState(false);
	const [metadataLeftBoxVisible, setMetadataLeftBoxVisible] =
		createState(false);
	const [bottomLayoutBotVisible, setBottomLayoutBotVisible] =
		createState(false);

	const [
		controlsLeftPlayPauseButtonVisible,
		setControlsLeftPlayPauseButtonVisible,
	] = createState(false);
	const [
		controlsLeftSeekNextButtonVisible,
		setControlsLeftSeekNextButtonVisible,
	] = createState(false);
	const [
		controlsLeftSeekPreviousButtonVisible,
		setControlsLeftSeekPreviousButtonVisible,
	] = createState(false);
	const [controlsLeftLoopButtonVisible, setControlsLeftLoopButtonVisible] =
		createState(false);
	const [
		controlsLeftShuffleButtonVisible,
		setControlsLeftShuffleButtonVisible,
	] = createState(false);
	const [
		controlsLeftVolumeButtonVisible,
		setControlsLeftVolumeButtonVisible,
	] = createState(false);

	const [
		bottomLayoutPlayPauseButtonVisible,
		setBottomLayoutPlayPauseButtonVisible,
	] = createState(false);
	const [
		bottomLayoutSeekNextButtonVisible,
		setBottomLayoutSeekNextButtonVisible,
	] = createState(false);
	const [
		bottomLayoutSeekPreviousButtonVisible,
		setBottomLayoutSeekPreviousButtonVisible,
	] = createState(false);
	const [bottomLayoutLoopButtonVisible, setBottomLayoutLoopButtonVisible] =
		createState(false);
	const [
		bottomLayoutShuffleButtonVisible,
		setBottomLayoutShuffleButtonVisible,
	] = createState(false);
	const [
		bottomLayoutVolumeButtonVisible,
		setBottomLayoutVolumeButtonVisible,
	] = createState(false);
	...
	return (
		...
		>
			...
			>
				<Adw.BreakpointBin widthRequest={380} heightRequest={60}>
					<Adw.Breakpoint
						condition={Adw.BreakpointCondition.new_length(
							Adw.BreakpointConditionLengthType.MAX_HEIGHT,
							130,
							Adw.LengthUnit.PX,
						)}
						onApply={() => {
					...
					<Adw.Breakpoint
						condition={Adw.BreakpointCondition.new_and(
							Adw.BreakpointCondition.new_length(
								Adw.BreakpointConditionLengthType.MIN_HEIGHT,
								131,
								Adw.LengthUnit.PX,
							),
							Adw.BreakpointCondition.new_length(
					...
					<With value={players}>
						{(players) => {
							...
							const playbackStatus = createBinding(
								mainPlayer,
								"playback_status",
							);
							const loopStatus = createBinding(
								mainPlayer,
								"loop_status",
							);
							const shuffleStatus = createBinding(
								mainPlayer,
								"shuffle_status",
							);
							const canGoNext = createBinding(
								mainPlayer,
								"can_go_next",
							);
							const canGoPrevious = createBinding(
								mainPlayer,
								"can_go_previous",
							);
							...
							function seekPrevious() {
								if (!mainPlayer || !mainPlayer.canGoPrevious)
									return;
							...
							function seekNext() {
								if (!mainPlayer || !mainPlayer.canGoNext)
									return;
							...
							function updateLoop() {
								...
								if (loopStatus === Mpris.Loop.NONE)
									return mainPlayer.set_loop_status(
										Mpris.Loop.PLAYLIST,
									);
								if (loopStatus === Mpris.Loop.PLAYLIST)
									return mainPlayer.set_loop_status(
							...
							function transformPlayPauseIcon(
							...
							function transformPlaybackClass(
								playbackStatus: Mpris.PlaybackStatus,
							) {
							...
							function transformPlaybackCursor(
								playbackStatus: Mpris.PlaybackStatus,
							) {
								if (
									!mainPlayer ||
							...
							function transformLoopIcon(loopStatus: Mpris.Loop) {
								...
								if (loopStatus === Mpris.Loop.TRACK)
									return "mi-repeat-one-symbolic";

								return "mi-repeat-symbolic";
							}

							function transformLoopClass(
								loopStatus: Mpris.Loop,
							) {
								const classes = ["large-icons"];
							...
							function transformShuffleClass(
								shuffleStatus: Mpris.Shuffle,
							) {
								const classes = ["large-icons"];
							...
							function transformShuffleCursor(
								shuffleStatus: Mpris.Shuffle,
							) {
								if (shuffleStatus === Mpris.Shuffle.UNSUPPORTED)
									return Gdk.Cursor.new_from_name(
							...
							function transformSeekNextClass(
								canGoNext: boolean,
							) {
								const classes = ["large-icons"];
							...
							function transformSeekNextCursor(
								canGoNext: boolean,
							) {
								if (!canGoNext)
									return Gdk.Cursor.new_from_name(
							...
							function transformSeekPreviousClass(
							...
							function transformSeekPreviousCursor(
								canGoPrevious: boolean,
							) {
								if (!canGoPrevious)
									return Gdk.Cursor.new_from_name(
							...
							return (
								<box>
									<box
										visible={controlsLeftBoxVisible}
										...
										>
											...
											<Adw.Breakpoint
												...
												onApply={() => {
													...
													setControlsLeftLoopButtonVisible(
														true,
													);
												}}
												onUnapply={() => {
											...
											>
												...
												<box
													...
													cursor={shuffleStatus(
														transformShuffleCursor,
													)}
												>
													<Gtk.GestureClick
														button={
												...
												<box
													...
													cursor={playbackStatus(
														transformPlaybackCursor,
													)}
												>
													<Gtk.GestureClick
														button={
													...
													/>
													<image
														iconName={playbackStatus(
															transformPlayPauseIcon,
														)}
														class={playbackStatus(
															transformPlaybackClass,
														)}
														widthRequest={
															buttonSize
														}
									...
									<box
										visible={metadataLeftBoxVisible}
									...
									<box
										visible={bottomLayoutBotVisible}
										orientation={Gtk.Orientation.VERTICAL}
										spacing={5}
									>
										<box>
											...
											>
												...
												<Adw.Breakpoint
													...
													onApply={() => {
														...
														setBottomLayoutLoopButtonVisible(
															true,
														);
													}}
													onUnapply={() => {
