...
import type { FormattedLyric, LyricsOutput, Metadata, MusixmatchSearchResult, SongData, TokenData, UsertokenResponse } from "@/types/lyrics";
...
async function _fetchLineSyncedLyricsMusixmatch(
    ...
    const url = new URL(`https://apic-desktop.musixmatch.com/ws/1.1/track.subtitle.get?${searchParams}`)
...
async function _searchLyricsNetease(
    ...
    const url = new URL(`https://music.xianqiao.wang/neteaseapiv2/search?${searchParams}`)
...
async function _fetchLyricsNetease(
    ...
    const url = new URL(`https://music.xianqiao.wang/neteaseapiv2/lyric?${searchParams}`)
...
async function fetchLyricsNetease(
    metadata: Metadata,
) {
    if (!metadata) return null;
