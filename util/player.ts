...
export function getMainPlayer(
	...
	const configData = config.peek();

	const preferredPlayers =
		configData.players?.preferred || defaultConfig.players.preferred;
	const blacklistedPlayers =
		configData.players?.blacklisted || defaultConfig.players.blacklisted;

	const filteredPlayers = players.filter((player) => {
		if (blacklistedPlayers.includes(player.identity)) return false;
		if (isPlaying && player.playbackStatus !== Mpris.PlaybackStatus.PLAYING)
			return false;
