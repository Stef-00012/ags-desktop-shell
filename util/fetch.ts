...
export class Response {
	...
	constructor(
		body: Gio.InputStream | null = null,
...
export async function fetch(
	url: string | URL,
	{ method, headers, body, flags }: RequestInit = {},
) {
	...
	const message = new Soup.Message({
		...
		uri:
			url instanceof URL
				? url.uri
				: GLib.Uri.parse(url, GLib.UriFlags.ENCODED),
	});
	...
	const inputStream: Gio.InputStream = await new Promise(
		(resolve, reject) => {
	...
	message
		...
		.foreach((name, value) => resposeHeaders.append(name, value));
